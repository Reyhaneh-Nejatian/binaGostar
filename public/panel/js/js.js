...
function create_custom_dropdowns() {
    $('select').each(function (i, select) {
        if (!$(this).next().hasClass('dropdown-select')) {
            $(this).after('<div class="dropdown-select wide ' + ($(this).attr('class') || '') + '" tabindex="0"><span class="current"></span>' +
                '<' + 'div class="list"><ul></ul></div></div>');
    ...
    if(confirm(message)){
        $.post(route, { _method: "PATCH", _token: $('meta[name="_token"]').attr('content') })
            ...
            .fail(function (response) {
                toastr.error('عملیات با موفقیت انجام نشد!', 'ناموفق');
...
function add(event){
    event.preventDefault();
    var attributeId = $('#attributes').val();
...
function updatePaymentStatus(event, route,message,status,field = 'status'){
